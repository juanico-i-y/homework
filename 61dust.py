...
parser.add_argument('--lowercase', action='store_true',
	help='change to lowercase masking')
arg = parser.parse_args()

#switch for lowercase or N mask

if arg.lowercase: 
	for name, seq in read_fasta(arg.fasta):
		rewrite = l_entropy_seq_rewriter(seq, arg.ethreshold, arg.winsize)
		print(f'>{name}')
	...
	for name, seq in read_fasta(arg.fasta):
		rewrite = n_entropy_seq_rewriter(seq, arg.ethreshold, arg.winsize)
		print(f'>{name}')
		for i in range(0, len(rewrite), arg.linelength):
			print(rewrite[i:i+arg.linelength])
